...
while getopts ":hu:t:i:l:" opt; do
  case "${opt}" in
    h)  echo "-t: tag name"
        ...
        echo "-l: local image name. Optional. Defaults to 'covid_upload_result'"
        exit
      ;;
    u) USER_NAME=${OPTARG}
      ;;    
    ...
    l) LOCAL_IMAGE_NAME=${OPTARG}
      ;;
    \? ) echo "Usage: cmd [-u] username  [-t] tag [-i] image [-b] base image tag [l] local image"
      exit
      ;;
  esac
done
...
if [ -z "${LOCAL_IMAGE_NAME}" ]; then
  LOCAL_IMAGE_NAME=covid_upload_result
fi
...
if [ -z "${IMAGE_NAME}" ]; then
  ...
  cat ~/my_password.txt | docker login -u ${USER_NAME} --password-stdin  
